...
const TASK_NAME_USER: &str = "Synclan";
const TASK_NAME_ADMIN: &str = "Synclan (Admin)";
const TASK_XML_DIR: &str = "tasks";
const TASK_XML_USER: &str = "synclan-task-user.xml";
const TASK_XML_ADMIN: &str = "synclan-task-admin.xml";
...
async fn cleanup_legacy_shortcuts() -> Result<()> {
    ...
    let shortcut = startup_dir.join("Synclan.lnk");

    shortcut.remove_if_exists().await?;
    Ok(())
}
